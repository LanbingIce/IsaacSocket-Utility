...
#define _MOD_CALLBACK(name)lua_State* L = local.isaac->luaVM->L;size_t top = local.lua.lua_gettop(L);local.lua.lua_getglobal(L, "Isaac");local.lua.lua_pushstring(L, "GetCallbacks");local.lua.lua_gettable(L, -2);local.lua.lua_pushstring(L, #name);local.lua.lua_pcall(L, 1, 1, 0);bool terminate = false;local.lua.lua_pushnil(L);while(local.lua.lua_next(L, -2) != 0){local.lua.lua_pushstring(L, "Function");local.lua.lua_gettable(L, -2);local.lua.lua_pushstring(L, "Mod");local.lua.lua_gettable(L, -3)
#define _MOD_CALLBACK_END()if(!local.lua.lua_isnil(L, -1)){terminate = true;break;}local.lua.lua_pop(L, 2);}local.lua.lua_settop(L, top);if(terminate){return 0;}
#define MOD_CALLBACK(name,param_type,param) _MOD_CALLBACK(name);local.lua.lua_push##param_type(L, param);local.lua.lua_pcall(L, 2, 1, 0);_MOD_CALLBACK_END()