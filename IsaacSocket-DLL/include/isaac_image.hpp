...
#pragma warning(disable: 4624)//禁用警告C4624: 已将析构函数隐式定义为“已删除”
    ...
    struct ModEntry {
        ...
        {
            std::string dir;
            FIELD(0x18, std::string, name);
            FIELD(0xFC, std::string, isEnabled);
            FIELD(0x120, std::string, version);
            FIELD(0x144, std::string, id);
        };
    };
    ...
    //游戏选项
    struct Options {
        std::string OptionsIniFilePath; // 0x0
    ...
    {
        char pad_0x2C[0x2C];
        std::vector<GlitchedEvent*> glitchedEvents;
    };
    ...
    struct Console
    {
        union
        {
            ...
            FIELD(0x3C, std::string, consoleText);
        };
    };
    ...
    struct Game
    {
        union
        {
            FIELD(0x1BA50, std::vector<Player*>, players);
            ...
            FIELD(0x283C20, std::vector<GlitchedItem*>, glitchedItems);
        };
    };
