...
#define MODULE_BEGIN(name) lua_State* L = local.isaac->luaEngine->L; int top = lua_gettop(L); lua_getglobal(L, "_ISAAC_SOCKET"); lua_pushstring(L, "IsaacSocket"); lua_gettable(L, -2); lua_pushstring(L, #name); lua_newtable(L)
...
#define _MOD_CALLBACK_BEGIN(name)lua_State* L = local.isaac->luaEngine->L;size_t top = lua_gettop(L);lua_getglobal(L, "Isaac");lua_pushstring(L, "GetCallbacks");lua_gettable(L, -2);lua_pushstring(L, #name);_LUA_PCALL(1, 1);lua_pushnil(L);while(lua_next(L, -2) != 0){lua_pushstring(L, "Function");lua_gettable(L, -2);lua_pushstring(L, "Mod");lua_gettable(L, -3);size_t paramNum = 1
