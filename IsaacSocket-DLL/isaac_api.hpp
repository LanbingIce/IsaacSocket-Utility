...
namespace isaac_api {
	...
	static int ReloadLua(lua_State* L) {
		ARG_DEF(1, boolean, bool, luaDebug, isaac->luaVM->luaDebug);
		isaac->luaVM->luaDebug = luaDebug;
	...
	static int GetActive(lua_State* L) {
		...
		lua->lua_newtable(L);
#define _(type,name) lua->lua_pushstring(L,#name); lua->lua_push##type(L, isaac->game->players[playerId]->actives[activeId].name); lua->lua_settable(L, -3)
		_(integer, item);
		_(integer, charge);
...
#undef _
	...
	static int SetActive(lua_State* L) {
		...
		{
			return lua->luaL_error(L, "bad argument #3 :active should be table");
		}
#define _(luaType,type,name) lua->lua_pushstring(L, #name); lua->lua_gettable(L, 3); if (lua->lua_is##luaType(L, -1)){isaac->game->players[playerId]->actives[activeId].name = (type)lua->lua_to##luaType(L, -1);}
...
#undef _
	...
	static int UnlockAchievement(lua_State* L) {
		...
		ARG_RANGE(achievementId, std::size(isaac->fileManager->achievements));
		ARG_DEF(2, boolean, bool, unlock, true);
		if (achievementId == 0)
		{
			for (size_t i = 1; i < std::size(isaac->fileManager->achievements); i++)
			{
				isaac->fileManager->achievements[i] = unlock;
			}
		}
		else
		{
			isaac->fileManager->achievements[achievementId] = unlock;
		}
		return 0;
	}
	...
	static int IsAchievementUnlocked(lua_State* L) {
		...
		if (achievementId == 0)
		{
			for (size_t i = 1; i < std::size(isaac->fileManager->achievements); i++)
			{
				if (!isaac->fileManager->achievements[i])
				{
					RET(boolean, false);
				}
			}
		...
		RET(boolean, isaac->fileManager->achievements[achievementId]);
	}

	//获取伊甸币
	static int GetEdenTokens(lua_State* L) {
		RET(integer, isaac->fileManager->edenTokens);
	}

	//设置伊甸币
	static int SetEdenTokens(lua_State* L) {
		ARG(1, integer, uint32_t, tokens);
		isaac->fileManager->edenTokens = tokens;
		return 0;
	}
	...
	static void Init(isaac::IsaacImage* isaac, lua::Lua* lua, state::StateData* stateData) {
		lua_State* L = isaac->luaVM->L;
		size_t top = lua->lua_gettop(L);

		isaac_api::isaac = isaac;
		isaac_api::lua = lua;
		isaac_api::stateData = stateData;

		lua->lua_getglobal(L, "_ISAAC_SOCKET");
		lua->lua_pushstring(L, "IsaacSocket");
		lua->lua_gettable(L, -2);
		lua->lua_pushstring(L, "IsaacAPI");
		lua->lua_newtable(L);
