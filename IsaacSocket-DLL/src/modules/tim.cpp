...
namespace tim {
	...
	static int TIMLogin(lua_State* L) {
		...
		RET(integer, ::TIMLogin(user_id, user_sig, local.callbacks.TIMCommCallback, nullptr));
	}
	...
	static int TIMMsgSendMessage(lua_State* L) {
		...
		local.lua.lua_pushinteger(L, ::TIMMsgSendMessage(conv_id, conv_type, json_msg_param, message_id_buffer, local.callbacks.TIMCommCallback, nullptr));
