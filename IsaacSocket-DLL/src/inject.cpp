...
namespace inject {
    ...
    //注入代码
    static void _InjectCode(size_t offset, LPCVOID fuctionAddress, size_t paddingSize)
    {
        size_t opSize = 1 + 4; //操作码长度为1，操作数长度为4
        uint8_t* injectAddress = (uint8_t*)&isaac + offset;
    ...
    __declspec(naked) void FASTCALL ExecuteCommand(const isaac_image::Console& console, LPCVOID, string& text, int unknow, LPCVOID unknow_point_guess)
    {
        __asm {
            push ebp
            mov ebp, esp
    ...
    __declspec(naked) void FASTCALL ConsoleOutput(const isaac_image::Console& console, LPCVOID, string& text, uint32_t color, int type_guess) {
        __asm {
            push ebp
            mov ebp, esp
            push - 0x01
...
#endif
        // SwapBuffers
        _InjectCode(0x4B1076, _SwapBuffers, 1);
        // 执行控制台指令
        _InjectCode(0x2655C5, _ExecuteCommand, 0);
        // 控制台输出
        _InjectCode(0x26AEC5, _ConsoleOutput, 0);
        // 日志输出
        _InjectCode(0x55E330, LogPrintf, 1);
        // MT19937随机数生成
        _InjectCode(0x2C3EA8, _MTRandom, 1);
        // 窗口消息
        _InjectCode(0x5971D0, WndProc, 1);
    }
}